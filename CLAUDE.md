...
| `/priceplan`, `💰 Запросы цены` | Поиск в разделе «Планирование → Запросы цены» |
| `/filters`, `⚙️ Фильтры поиска` | ФЗ, ОКПД2, ключевые слова, заказчик, тип даты, сумма, пресеты |
...
`date_type`: `"published"` (по размещению) | `"updated"` (по обновлению) | `"end"` (по окончанию подачи).
`customer_inn`: ИНН или часть наименования заказчика → параметр ЕИС `customerFullNameOrinn`.

## Фильтры запросов цены

Структура (`priceplan_filter.json`) — без суммы НМЦ:
...
- **Раздел «Запросы цены» — CSS-селекторы** могут отличаться от основного раздела. При первом запуске `parsers/priceplan.py` сохраняет `data/debug_priceplan.html` для проверки. Если карточки не парсятся — смотреть этот файл и корректировать селекторы в `parse_priceplan_results()`.
