...
│   └── config.json           # username бота и webapp_url (генерируется, не в git)
├── data/
...
bot/bot.py  (Telegram UI)
    │
    ├── После поиска → пушит data_{chat_id}.json на GitHub через API
    │                           ↓
...
- `webapp/index.html` хостится на **GitHub Pages** — постоянный URL, не зависит от рестарта бота
- После каждого поиска бот пушит `data_{chat_id}.json` / `subs_{chat_id}.json` в репозиторий через GitHub API
- GitHub Pages раздаёт файлы — Mini App подтягивает их при открытии
...
| Кнопка "🌐 Приложение" | Открыть Telegram Mini App (главное меню, всегда доступна) |
...
Открывается кнопкой «🌐 Приложение» в главном меню (всегда доступна, не требует предварительного поиска).
...
**Данные:** после каждого поиска бот пушит `data_{chat_id}.json` в репозиторий через GitHub API — файл появляется на Pages через несколько секунд. Каждый пользователь видит только свои данные.
...
1. **Обновляет документацию** — CLAUDE.md и все файлы в `docs/`, которые затронуты изменениями сессии (структура, баги, команды, архитектура)
