...
│   ├── priceplan_filter.json # фильтры для раздела НМЦК
│   ├── saved_filters.json    # пресеты фильтров бота (закупки)
...
| `/filters`, `⚙️ Фильтры поиска` (раздел Закупки) | ФЗ, способ закупки, ОКПД2, ключевые слова, заказчик, тип даты, сумма, пресеты |
...
| `/ppfilters`, `⚙️ Фильтры поиска` (раздел НМЦК) | Фильтры НМЦК: статус, даты, слова, заказчик |
| `/watch_pp`, `🔔 Подписки` (раздел НМЦК) | Авто-уведомления по запросам цены |
